(function() {
  ...
  // 创建 API 请求 URL
  const apiUrl = `${this.config.aiApi}?q=${encodeURIComponent(title)}&content=${encodeURIComponent(content.substring(0, 400))}&lang=en`;

  // 显示加载状态
  const loadingText = isEnglishPage ? 'Generating summary...' : '正在生成摘要...';
  summaryContent.innerHTML = `<div class="ai-summary-loading">${loadingText}</div>`;
  ...
  fetch(apiUrl)
    ...
    .then(data => {
      if (data && data.summary) {
        ...
        if (isEnglishPage) {
          // 如果是英文页面，直接显示英文摘要
          summaryContent.innerHTML = `<div class="ai-summary-content">${englishSummary}</div>`;
        } else {
          // 如果是中文页面，翻译成中文后显示
          this.translateSummary(englishSummary, summaryContent);
        }
      } else {
